...
def generate_logs_folder(base_folder_name, index):
    sub_folders = [util.NETWORK_BEFORE_FOLDER, util.NETWORK_AFTER_FOLDER]

    indexed_folder = os.path.join(os.getcwd(), base_folder_name, util.CRAWLED_NETWORK_LOGS_FOLDER, index)
...
def intercept_network_request(page, base_folder_name, index, before_after_flag):
    ...
    request_save_loc = os.path.join(logs_folder_path, before_after_flag, util.NETWORK_LOGS_REQUEST_FILE)
    response_save_loc = os.path.join(logs_folder_path, before_after_flag, util.NETWORK_LOGS_RESPONSE_FILE)
    console_save_loc = os.path.join(logs_folder_path, before_after_flag, util.NETWORK_LOGS_DOM_FUNCTION_FILE)
