...
def get_before_client_side_rendering_data(page, base_folder_name, formatted_index):
    ...
    crawler_support.save_crawled_url(base_folder_name, page.url, formatted_index, definitions.SUBFOLDER_BEFORE)
...
def get_html_content(page, base_folder_name, formatted_index, actual_url, action_flag, embedded_flag):
    ...
    crawler_support.detect_redirection(base_folder_name, scraped_url, actual_url, formatted_index)
    crawler_support.save_crawled_url(base_folder_name, scraped_url, formatted_index, definitions.SUBFOLDER_AFTER)
...
def get_dataset(page, base_folder_name, url_list, referrer, action_flag):
    ...
    for url in url_list:
        ...
        try:
            ...
            if content is not None:
                crawler_support.save_html_script(base_folder_name, content, formatted_index_str)
            ...
            crawler_support.save_html_script(base_folder_name, f"Error occurred for url: {url}\n{e}", formatted_index_str, definitions.SUBFOLDER_AFTER)
            crawler_support.save_crawled_url(base_folder_name, definitions.ERROR_URL_FLAG, formatted_index_str, definitions.SUBFOLDER_AFTER)
            error_list.append(index)
            index += 1
            continue
...
crawl(["https://www.google.com/"], definitions.CONFIG_DESKTOP_USER, action_flag=True, referrer=definitions.GOOGLE_SEARCH_QUERY_REFERRER)